...
const fetchRegistration = async () => {
    ...
    axios.get(`https://vmtest-1-w8r3.onrender.com/api/vendor_admin/activity/registration?activityId=${activityId}`, {
        headers: {
...
const handleSubmit = async () => {
    ...
    try {
        if (operationType.value === 'confirm') {
            // 执行确认操作
            await axios.put(`https://vmtest-1-w8r3.onrender.com/api/vendor_admin/registration/confirmById/${selectedRegister.value.id}`);
            selectedRegister.value.status = "confirmed";
        } else if (operationType.value === 'cancel') {
            // 执行取消操作
            await axios.put(`https://vmtest-1-w8r3.onrender.com/api/vendor_admin/registration/cancelById/${selectedRegister.value.id}`);
            selectedRegister.value.status = "canceled";
        }

        // 发送通知
        await axios.post(`https://vmtest-1-w8r3.onrender.com/api/vendor_admin/registration/notification/${selectedRegister.value.member.id}/${activityId}`, null, {
            params: {
...
const handleBatchSubmit = async () => {
    ...
    try {
        const requests = selectedRegisters.value.map(async (register) => {
            if (operationType.value === 'confirm') {
                await axios.put(`https://vmtest-1-w8r3.onrender.com/api/vendor_admin/registration/confirmById/${register.id}`);
                register.status = "confirmed";
            } else if (operationType.value === 'cancel') {
                await axios.put(`https://vmtest-1-w8r3.onrender.com/api/vendor_admin/registration/cancelById/${register.id}`);
                register.status = "canceled";
            }

            // 發送通知
            return axios.post(`https://vmtest-1-w8r3.onrender.com/api/vendor_admin/registration/notification/${register.member.id}`, null, {
                params: {
...
const deleteRegistration = async (registerId) => {
    ...
    try {
        axios.delete(`https://vmtest-1-w8r3.onrender.com/api/vendor_admin/registration/deleteById/${registerId}`)
            .then(() => {
