...
const submitForm = () => {
    // 检查该店家是否已经申请过该认证标语
    axios.get(`https://pettopia-1743656430689.azurewebsites.net/api/vendor_admin/certification/exists/${vendorId.value}/${certificationTagId.value}`)
        .then(response => {
            if (response.data.exists) {
                ...
                axios.post('https://pettopia-1743656430689.azurewebsites.net/api/vendor_admin/certification/add', formdata)
                    .then(response => {
                        alert('申請成功');
...
const deleteCertification = (recordId) => {
    if (confirm("確定要取消申請嗎？")) {
        axios.delete(`https://pettopia-1743656430689.azurewebsites.net/api/vendor_admin/certification/delete/${recordId}`)
            .then(response => {
                alert('取消申請成功');
                // 删除成功后重新加载认证申请记录
                fetchCertification();
...
const fetchCertification = () => {
    // 獲取申請紀錄
    axios.get(`https://pettopia-1743656430689.azurewebsites.net/api/vendor_admin/certification/${vendorId.value}`)
        .then(response => {
...
onMounted(() => {
    ...
    axios.get('https://pettopia-1743656430689.azurewebsites.net/api/certification_type/all')
        .then(response => {
            console.log(response.data);
