...
const fetchEvents = async () => {
  try {
    const response = await axios.get(`https://pettopia-1743656430689.azurewebsites.net/api/vendor_admin/activity/${userId}`, {
      headers: {
  ...
  for (let event of events.value) {
    try {
      let response = await axios.get(
        `https://pettopia-1743656430689.azurewebsites.net/photos/ids?vendorActivityId=${event.id}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      }
...
const getImageBlob = async (photoId) => {
  try {
    let response = await axios.get(`https://pettopia-1743656430689.azurewebsites.net/photos/download?photoId=${photoId}`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`
      },
...
const getEventImageUrl = async (eventId) => {
  ...
  try {
    // 请求图片 ID 列表
    const response = await axios.get(`https://pettopia-1743656430689.azurewebsites.net/photos/ids?vendorActivityId=${eventId}`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
    })
...
const deleteEvent = async (activityId) => {
  try {
    await axios.delete(`https://pettopia-1743656430689.azurewebsites.net/${activityId}`)
