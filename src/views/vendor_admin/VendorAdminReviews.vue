...
const fetchReviews = async () => {
  try {
    const response = await axios.get(`https://pettopia-1743656430689.azurewebsites.net/api/vendor_admin/review?vendorId=${userId}`, {
...
async function getImageSrc(photoId) {
  try {
    const response = await axios.get(`https://pettopia-1743656430689.azurewebsites.net/review_photos/download?photoId=${photoId}`, {
      headers: {
...
const showPhotoModal = (photoId) => {
  ...
  const photoUrl = `https://pettopia-1743656430689.azurewebsites.net/review_photos/download?photoId=${photoId}`;
