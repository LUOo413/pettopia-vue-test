...
const calendarOptions = ref({
  ...
  eventDrop: async (info) => {
    try {
      ...
      // 调用后端 API 更新数据库
      const response = await axios.put(`https://pettopia-1743656430689.azurewebsites.net/api/vendor_admin/calendar/update/${updatedEvent.eventId}`, null, {
        params: updatedEvent
      });
...
const loadEvents = async () => {
  try {
    const response = await axios.get(`https://pettopia-1743656430689.azurewebsites.net/api/vendor_admin/calendar/${vendorId}`);
...
const addEvent = async () => {
  ...
  try {
    const response = await axios.post("https://pettopia-1743656430689.azurewebsites.net/api/vendor_admin/calendar/add", formData, {
      headers: {
...
const updateEvent = async () => {
  ...
  try {
    const response = await axios.put(`https://pettopia-1743656430689.azurewebsites.net/api/vendor_admin/calendar/update/${editEventId.value}`, null, {
      params: {
...
const deleteEvent = async () => {
  ...
  try {
    await axios.delete(`https://pettopia-1743656430689.azurewebsites.net/api/vendor_admin/calendar/delete/${editEventId.value}`);
    const calendarApi = calendar.value?.getApi();
    const event = calendarApi?.getEventById(editEventId.value);
    if (event) {
      event.remove();
